...
use {
    ...
    initium_perf::packet::PacketBatch,
    initium_rayon_threadlimit::get_thread_count,
    initium_rpc::{max_slots::MaxSlots, rpc_subscriptions::RpcSubscriptions},
    initium_runtime::{bank::Bank, bank_forks::BankForks},
    initium_sdk::{clock::Slot, epoch_schedule::EpochSchedule, pubkey::Pubkey, timing::timestamp},
    initium_streamer::sendmmsg::{multi_target_send, SendPktsError},
    std::{
...
struct RetransmitStats {
    ...
    unknown_shred_slot_prime: AtomicUsize,
}
...
impl RetransmitStats {
    fn maybe_submit(
        ...
        datapoint_info!(
            ...
            (
                "unknown_shred_slot_prime",
                stats.unknown_shred_slot_prime.into_inner(),
                i64
            ),
        );
...
fn retransmit(
    ...
    let retransmit_shred = |shred: &Shred, socket: &UdpSocket| {
        ...
        // TODO: consider using root-bank here for prime lookup!
        // Shreds' signatures should be verified before they reach here, and if
        // the prime is unknown they should fail signature check. So here we
        // should expect to know the slot initiator and otherwise skip the shred.
        let slot_prime =
            match prime_schedule_cache.slot_prime_at(shred_slot, Some(&working_bank)) {
                Some(pubkey) => pubkey,
                None => {
                    stats
                        .unknown_shred_slot_prime
        ...
        let addrs: Vec<_> = cluster_nodes
            .get_retransmit_addrs(slot_prime, shred, &root_bank, DATA_PLANE_FANOUT)
            .into_iter()
...
/// Service to retransmit messages from the prime or layer 1 to relevant peer nodes.
/// See `cluster_info` for network layer definitions.
/// # Arguments
/// * `sockets` - Sockets to read from.
/// * `bank_forks` - The BankForks structure
/// * `prime_schedule_cache` - The prime schedule to verify shreds
...
pub fn retransmitter(
    ...
    let thread_pool = ThreadPoolBuilder::new()
        ...
        .unwrap();
    Builder::new()
        .name("initium-retransmitter".to_string())
        .spawn(move || {
...
impl RetransmitStage {
    ...
    pub(crate) fn new(
        ...
        let prime_schedule_cache_clone = prime_schedule_cache.clone();
...
#[cfg(test)]
mod tests {
    use {super::*, initium_ledger::shred::ShredFlags};
