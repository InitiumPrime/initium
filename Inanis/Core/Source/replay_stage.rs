//! The `replay_stage` replays transactions broadcast by the Prime Node.
use {
    crate::{
    ...
    initium_client::rpc_response::SlotUpdate,
    initium_entry::entry::VerifyRecyclers,
    initium_geyser_plugin_manager::block_metadata_notifier_interface::BlockMetadataNotifierLock,
    initium_gossip::cluster_info::ClusterInfo,
    initium_ledger::{
        block_error::BlockError,
        blockstore::Blockstore,
        blockstore_processor::{self, BlockstoreProcessorError, TransactionStatusSender},
        prime_schedule_cache::PrimeScheduleCache,
        prime_schedule_utils::first_of_consecutive_prime_slots,
    },
    initium_measure::measure::Measure,
    initium_metrics::inc_new_counter_info,
    initium_posynch::posynch_recorder::{PosyncPrimeStatus, PosyncRecorder, GRACE_TICKS_FACTOR, MAX_GRACE_SLOTS},
    initium_program_runtime::timings::ExecuteTimings,
    initium_rpc::{
    ...
    initium_sdk::{
        clock::{BankId, Slot, MAX_PROCESSING_AGE, NUM_CONSECUTIVE_PRIME_SLOTS},
    ...
    initium_vote_program::vote_state::VoteTransaction,
    std::{
...
pub struct ReplayStageConfig {
    ...
    pub prime_schedule_cache: Arc<PrimeScheduleCache>,
    ...
    pub wait_for_vote_to_start_prime: bool,
...
impl ReplayTiming {
    ...
    fn update(
        ...
        self.start_prime_elapsed += start_prime_elapsed;
        ...
        if elapsed_ms > 1000 {
            ...
            datapoint_info!(
                ...
                (
                    "start_prime_elapsed",
                    self.start_prime_elapsed as i64,
                    i64
                ),
...
impl ReplayStage {
    ...
    pub fn new<T: Into<Tower> + Sized>(
        ...
        posync_recorder: Arc<Mutex<PosyncRecorder>>,
        tower: T,
        ...
        let t_replay = Builder::new()
            .name("initium-replay-stage".to_string())
            .spawn(move || {
                ...
                let mut has_new_vote_been_rooted = !wait_for_vote_to_start_prime;
                ...
                loop {
                    ...
                    let mut tpu_has_bank = posync_recorder.lock().unwrap().has_bank();
                    ...
                    if tower.is_recent(heaviest_bank.slot()) && !heaviest_fork_failures.is_empty() {
                        ...
                        for r in heaviest_fork_failures {
                            if let HeaviestForkFailures::NoPropagatedConfirmation(slot) = r {
                                if let Some(latest_prime_slot) =
                                    progress.get_latest_prime_slot_must_exist(slot)
                                {
                                    progress.log_propagated_stats(latest_prime_slot, &bank_forks);
                                }
                            }
                        }
                    }
                    ...
                    if let Some((ref vote_bank, ref switch_fork_decision)) = vote_bank {
                        if let Some(votable_prime) =
                            prime_schedule_cache.slot_prime_at(vote_bank.slot(), Some(vote_bank))
                        {
                            Self::log_prime_change(
                                &my_pubkey,
                                vote_bank.slot(),
                                &mut current_prime,
                                &votable_prime,
                    ...
                    if let Some(reset_bank) = reset_bank {
                        if last_reset != reset_bank.last_blockhash() {
                            ...
                            if my_pubkey != cluster_info.id() {
                                ...
                                // becoming prime node
                                has_new_vote_been_rooted = !wait_for_vote_to_start_prime;
                    ...
                    //
                    // Has to be before `maybe_start_prime()`. Otherwise, `ancestors` and `descendants`
                    // will be outdated, and we cannot assume `posync_bank` will be in either of these maps.
                    Self::dump_then_repair_correct_slots(&mut duplicate_slots_to_repair, &mut ancestors, &mut descendants, &mut progress, &bank_forks, &blockstore, posynch_bank.map(|bank| bank.slot()));
                    ...
                    Self::retransmit_latest_unpropagated_prime_slot(
                        &posync_recorder,
                    ...
                    // From this point on, its not safe to use ancestors/descendants since maybe_start_prime
                    // may add a bank that will not included in either of these maps.
                    drop(ancestors);
                    drop(descendants);
                    if !tpu_has_bank {
                        Self::maybe_start_prime(
                            &my_pubkey,
                            &bank_forks,
                            &posync_recorder,
                            &prime_schedule_cache,
    ...
    fn maybe_retransmit_unpropagated_slots(
        ...
        latest_prime_slot: Slot,
    ) {
        let first_prime_group_slot = first_of_consecutive_prime_slots(latest_prime_slot);

        for slot in first_prime_group_slot..=latest_prime_slot {
            ...
            if let Some(retransmit_info) = progress.get_retransmit_info_mut(slot) {
                if !is_propagated.expect(
                    ...
                    if retransmit_info.reached_retransmit_threshold() {
                        info!(
                            "Retrying retransmit: latest_prime_slot={} slot={} retransmit_info={:?}",
                            latest_prime_slot,
                            slot,
                            &retransmit_info,
                        );
                        datapoint_info!(
                            metric_name,
                            ("latest_prime_slot", latest_prime_slot, i64),
                            ("slot", slot, i64),
    ...
    fn retransmit_latest_unpropagated_prime_slot(
        posync_recorder: &Arc<Mutex<PosyncRecorder>>,
        retransmit_slots_sender: &RetransmitSlotsSender,
        progress: &mut ProgressMap,
    ) {
        let start_slot = posync_recorder.lock().unwrap().start_slot();

        if let (false, Some(latest_prime_slot)) =
            progress.get_prime_propagation_slot_must_exist(start_slot)
        {
            debug!(
                "Slot not propagated: start_slot={} latest_prime_slot={}",
                start_slot, latest_prime_slot
            );
    ...
    pub fn initialize_progress_and_fork_choice(
        ...
        for bank in &frozen_banks {
            let prev_prime_slot = progress.get_bank_prev_prime_slot(bank);
            progress.insert(
                bank.slot(),
                ForkProgress::new_from_bank(bank, my_pubkey, vote_account, prev_prime_slot, 0, 0),
            );
        }
    ...
    pub fn dump_then_repair_correct_slots(
        ...
        duplicate_slots_to_repair.retain(|duplicate_slot, correct_hash| {
            // Should not purge duplicate slots if there is currently a posync bank building
            ...
            // 2) And also called `reset_posync_recorder()`
            // Then we should have reset to a fork that doesn't include the duplicate block,
            // which means any working bank in PosyncRecorder that was built on that duplicate fork
            // should have been cleared as well. However, if there is some violation of this guarantee,
            // then log here
            let is_posync_building_on_duplicate_fork = posync_bank_slot
                .map(|posync_bank_slot| {
                    ancestors
                        .get(&posync_bank_slot)
                        .expect("Posync bank should exist in BankForks and thus in ancestors map")
                        .contains(duplicate_slot)
                })
                .unwrap_or(false);

            let did_purge_repair = {
                if !is_posync_building_on_duplicate_fork {
                ...
                // `duplicate_slot` with hash == `correct_hash`
                    ...
                    warn!(
                        "PoSynch bank for slot {} is building on duplicate slot {}",
                        posync_bank_slot.unwrap(),
                        duplicate_slot
                    );
                    false
                }
    ...
    fn log_prime_change(
        my_pubkey: &Pubkey,
        bank_slot: Slot,
        current_prime: &mut Option<Pubkey>,
        new_prime: &Pubkey,
    ) {
        if let Some(ref current_prime) = current_prime {
            if current_prime != new_prime {
                let msg = if current_prime == my_pubkey {
                    ". I am no longer the Prime"
                } else if new_prime == my_pubkey {
                    ". I am now the Prime"
                } else {
                    ""
                };
                info!(
                    "PRIME CHANGE at slot: {} prime: {}{}",
                    bank_slot, new_prime, msg
                );
            }
        }
        current_prime.replace(new_prime.to_owned());
    }

    fn check_propagation_for_start_prime(
        posync_slot: Slot,
        parent_slot: Slot,
        progress_map: &ProgressMap,
    ) -> bool {
        // Assume `NUM_CONSECUTIVE_PRIME_SLOTS` = 4. Then `skip_propagated_check`
        // below is true if `posync_slot` is within the same `NUM_CONSECUTIVE_PRIME_SLOTS`
        // set of blocks as `latest_prime_slot`.
        //
        // Example 1 (`posync_slot` directly descended from `latest_prime_slot`):
        //
        // [B B B B] [B B B latest_prime_slot] posync_slot
        //
        // Example 2:
        //
        // [B latest_prime_slot B posync_slot]
        //
        // In this example, even if there's a block `B` on another fork between
        // `posync_slot` and `parent_slot`, because they're in the same
        // `NUM_CONSECUTIVE_PRIME_SLOTS` block, we still skip the propagated
        // check because it's still within the propagation grace period.
        if let Some(latest_prime_slot) =
            progress_map.get_latest_prime_slot_must_exist(parent_slot)
        {
            let skip_propagated_check =
                posync_slot - latest_prime_slot < NUM_CONSECUTIVE_PRIME_SLOTS;
        ...
        // of `parent_slot` (hence the call to `get_latest_prime_slot()` in the
        // check above)
        progress_map
            .get_prime_propagation_slot_must_exist(parent_slot)
            .0
    }

    fn should_retransmit(posync_slot: Slot, last_retransmit_slot: &mut Slot) -> bool {
        if posync_slot < *last_retransmit_slot
            || posync_slot >= *last_retransmit_slot + NUM_CONSECUTIVE_PRIME_SLOTS
        {
            *last_retransmit_slot = posync_slot;
            true
        } else {
            false
        }
    ...
    #[allow(clippy::too_many_arguments)]
    fn maybe_start_prime(
        my_pubkey: &Pubkey,
        bank_forks: &Arc<RwLock<BankForks>>,
        posync_recorder: &Arc<Mutex<PosyncRecorder>>,
        prime_schedule_cache: &Arc<PrimeScheduleCache>,
        ...
        // all the individual calls to posync_recorder.lock() are designed to
        // increase granularity, decrease contention

        assert!(!posync_recorder.lock().unwrap().has_bank());

        let (posync_slot, parent_slot) = match posync_recorder.lock().unwrap().reached_prime_slot() {
            PosyncPrimeStatus::Reached {
                posync_slot,
                parent_slot,
            } => (posync_slot, parent_slot),
            PosyncPrimeStatus::NotReached => {
                trace!("{} posync_recorder hasn't reached_prime_slot", my_pubkey);
                return;
            }
        };

        trace!("{} reached_prime_slot", my_pubkey);
        ...
        if bank_forks.read().unwrap().get(posync_slot).is_some() {
            warn!("{} already have bank in forks at {}?", my_pubkey, posync_slot);
            return;
        }
        trace!(
            "{} posync_slot {} parent_slot {}",
        ...
        if let Some(next_prime) = prime_schedule_cache.slot_prime_at(posync_slot, Some(&parent)) {
            if !has_new_vote_been_rooted {
                info!("Haven't landed a vote, so skipping my prime slot");
                return;
            }

            trace!(
                "{} prime {} at posync slot: {}",
                my_pubkey,
                next_prime,
                posync_slot
            );
            ...
            if next_prime != *my_pubkey {
                return;
            }

            datapoint_info!(
                "replay_stage-new_prime",
                ("slot", posync_slot, i64),
                ("prime", next_prime.to_string(), String),
            );

            if !Self::check_propagation_for_start_prime(posync_slot, parent_slot, progress_map) {
                let latest_unconfirmed_prime_slot = progress_map.get_latest_prime_slot_must_exist(parent_slot)
                    .expect("In order for propagated check to fail, latest prime must exist in progress map");
                if posync_slot != skipped_slots_info.last_skipped_slot {
                    datapoint_info!(
                        "replay_stage-skip_prime_slot",
                        ("slot", posync_slot, i64),
                        ("parent_slot", parent_slot, i64),
                        (
                            "latest_unconfirmed_prime_slot",
                            latest_unconfirmed_prime_slot,
                            i64
                        )
                    );
                    progress_map.log_propagated_stats(latest_unconfirmed_prime_slot, bank_forks);
                    skipped_slots_info.last_skipped_slot = posync_slot;
                }
                if Self::should_retransmit(posync_slot, &mut skipped_slots_info.last_retransmit_slot) {
                    Self::maybe_retransmit_unpropagated_slots(
            ...
            datapoint_info!("replay_stage-my_prime_slot", ("slot", posync_slot, i64),);
            info!(
                "new fork:{} parent:{} (prime) root:{}",
                posync_slot, parent_slot, root_slot
            );

            let root_distance = posync_slot - root_slot;
            let vote_only_bank = if root_distance > MAX_ROOT_DISTANCE_FOR_VOTE_ONLY {
                datapoint_info!("vote-only-bank", ("slot", posync_slot, i64));
                true
            } else {
                false
            };
            ...
            posync_recorder.lock().unwrap().set_bank(&tpu_bank);
        } else {
            error!("{} No next Prime Node found", my_pubkey);
        }
    }
    ...
    fn handle_votable_bank(
        ...
        if let Some(new_root) = new_root {
            ...
            // Call prime schedule_cache.set_root() before blockstore.set_root() because
            // bank_forks.root is consumed by repair_service to update gossip, so we don't want to
            // get shreds for repair on gossip before we update prime schedule, otherwise they may
            // get dropped.
            prime_schedule_cache.set_root(rooted_banks.last().unwrap());
            blockstore
    ...
    fn reset_posync_recorder(
        my_pubkey: &Pubkey,
        blockstore: &Blockstore,
        bank: &Arc<Bank>,
        posync_recorder: &Mutex<PosyncRecorder>,
        prime_schedule_cache: &PrimeScheduleCache,
    ) {
        let next_prime_slot = prime_schedule_cache.next_prime_slot(
            my_pubkey,
            bank.slot(),
            bank,
            Some(blockstore),
        ...
        posync_recorder
            .lock()
            .unwrap()
            .reset(bank.clone(), next_prime_slot);

        let next_prime_msg = if let Some(next_prime_slot) = next_prime_slot {
            format!("My next Prime slot is {}", next_prime_slot.0)
        } else {
            "I am not in the Prime schedule yet".to_owned()
        };

        info!(
            "{} reset PoSync to tick {} (within slot {}). {}",
            my_pubkey,
            bank.tick_height(),
            bank.slot(),
            next_prime_msg,
    ...
    fn replay_active_banks(
        ...
        for bank_slot in &active_banks {
            ...
            // Insert a progress entry even for slots this node is the prime for, so that
    ...
    pub fn compute_bank_stats(
        ...
        for bank in frozen_banks {
            ...
            // is if this node was the prime for this slot as those banks
    ...
    fn update_propagation_status(
        ...
        if progress.get_prime_propagation_slot_must_exist(slot).0 {
            return;
        }
    ...
    pub fn select_vote_and_reset_forks(
        ...
        // 2) Will reset PoSync to heaviest fork in order to make sure the heaviest
        //    fork is propagated
        // This above behavior should ensure correct voting and resetting PoSync
        ...
        if let Some((bank, switch_fork_decision)) = selected_fork {
            let (is_locked_out, vote_threshold, is_prime_slot, fork_weight) = {
                ...
                (
                    ...
                    propagated_stats.is_prime_slot,
                    fork_stats.weight,
                )
            };

            let propagation_confirmed = is_prime_slot
                || progress
                    .get_prime_propagation_slot_must_exist(bank.slot())
                    .0;
    ...
    fn update_fork_propagated_threshold_from_votes(
        ...
        let mut current_prime_slot = progress.get_latest_prime_slot_must_exist(fork_tip);
        let mut did_newly_reach_threshold = false;
        let root = bank_forks.read().unwrap().root();
        loop {
            // These cases mean confirmation of propagation on any earlier
            // prime blocks must have been reached
            if current_prime_slot == None || current_prime_slot.unwrap() < root {
                break;
            }

            let prime_propagated_stats = progress
                .get_propagated_stats_mut(current_prime_slot.unwrap())
                .expect("current_prime_slot >= root, so must exist in the progress map");
            ...
            // We only iterate through the list of prime slots by traversing
            // the linked list of 'prev_prime_slot`'s outlined in the
            // `progress` map
            assert!(prime_propagated_stats.is_prime_slot);
            let prime_bank = bank_forks
                .read()
                .unwrap()
                .get(current_prime_slot.unwrap())
            ...
            did_newly_reach_threshold = Self::update_slot_propagated_threshold_from_votes(
                ...
                &prime_bank,
                prime_propagated_stats,
                did_newly_reach_threshold,
            ) || did_newly_reach_threshold;

            // Now jump to process the previous prime slot
            current_prime_slot = prime_propagated_stats.prev_prime_slot;
        }
    }
    ...
    fn update_slot_propagated_threshold_from_votes(
        ...
        prime_bank: &Bank,
        prime_propagated_stats: &mut PropagatedStats,
        ...
        if did_child_reach_threshold {
            if !prime_propagated_stats.is_propagated {
                prime_propagated_stats.is_propagated = true;
                return true;
            } else {
                return false;
            }
        ...
        newly_voted_pubkeys.retain(|vote_pubkey| {
            let exists = prime_propagated_stats
                .propagated_validators
                .contains(vote_pubkey);
            prime_propagated_stats.add_vote_pubkey(
                *vote_pubkey,
                prime_bank.epoch_vote_account_stake(vote_pubkey),
            );
            !exists
        });
        ...
        cluster_slot_pubkeys.retain(|node_pubkey| {
            ...
            prime_propagated_stats.add_node_pubkey(&*node_pubkey, prime_bank);
            !exists
        });

        if prime_propagated_stats.total_epoch_stake == 0
            || prime_propagated_stats.propagated_validators_stake as f64
                / prime_propagated_stats.total_epoch_stake as f64
    ...
    fn generate_new_bank_forks(
        ...
        for (parent_slot, children) in next_slots {
            ...
            for child_slot in children {
                ...
                let prime = prime_schedule_cache
                    .slot_prime_at(child_slot, Some(&parent_bank))
                    .unwrap();
                info!(
    ...
    fn new_bank_from_parent_with_notify(
        ...
        Bank::new_from_parent_with_options(parent, prime, slot, new_bank_options)
    }
...
pub(crate) mod tests {
    use {
        ...
        initium_entry::entry::{self, Entry},
        initium_gossip::{cluster_info::Node, crds::Cursor},
        initium_prime::{
            blockstore::{entries_to_test_shreds, make_slot_entries, BlockstoreError},
            create_new_tmp_ledger,
            genesis_utils::{create_genesis_config, create_genesis_config_with_prime},
            get_tmp_ledger_path,
        ...
        initium_sdk::{
            clock::NUM_CONSECUTIVE_PRIME_SLOTS,
            genesis_config,
            hash::{hash, Hash},
            instruction::InstructionError,
            posync_config::PosyncConfig,
            signature::{Keypair, Signer},
            system_transaction,
            transaction::TransactionError,
        },
        initium_streamer::socket::SocketAddrSpace,
        initium_transaction_status::VersionedTransactionWithStatusMeta,
        initium_vote_program::{
    ...
    pub fn replay_blockstore_components(
        ...
        // Prime schedule cache
        let root_bank = bank_forks.read().unwrap().root_bank();
        let prime_schedule_cache = Arc::new(PrimeScheduleCache::new_from_bank(&root_bank));

        // PosyncRecorder
        let working_bank = bank_forks.read().unwrap().working_bank();
        let posync_recorder = Mutex::new(
            PosyncRecorder::new(
    ...
    fn test_child_slots_of_same_parent() {
        ...
        let bank1 = Bank::new_from_parent(
            ...
            &prime_schedule_cache.slot_prime_at(1, None).unwrap(),
            1,
        );
        progress.insert(
            1,
        ...
        assert!(progress.get_propagated_stats(1).unwrap().is_prime_slot);
        bank1.freeze();
        bank_forks.write().unwrap().insert(bank1);

        // Insert shreds for slot NUM_CONSECUTIVE_PRIME_SLOTS,
        // chaining to slot 1
        let (shreds, _) = make_slot_entries(NUM_CONSECUTIVE_PRIME_SLOTS, 1, 8);
        blockstore.insert_shreds(shreds, None, false).unwrap();
        assert!(bank_forks
            .read()
            .unwrap()
            .get(NUM_CONSECUTIVE_PRIME_SLOTS)
            .is_none());
        let mut replay_timing = ReplayTiming::default();
        ...
        // Insert shreds for slot 2 * NUM_CONSECUTIVE_PRIME_SLOTS,
        // chaining to slot 1
        let (shreds, _) = make_slot_entries(2 * NUM_CONSECUTIVE_PRIME_SLOTS, 1, 8);
        blockstore.insert_shreds(shreds, None, false).unwrap();
        assert!(bank_forks
            .read()
            .unwrap()
            .get(2 * NUM_CONSECUTIVE_PRIME_SLOTS)
            .is_none());
        ReplayStage::generate_new_bank_forks(
            &blockstore,
            &bank_forks,
        ...
        let expected_prime_slots = vec![
            1,
            NUM_CONSECUTIVE_PRIME_SLOTS,
            2 * NUM_CONSECUTIVE_PRIME_SLOTS,
        ];
        for slot in expected_prime_slots {
            let prime = prime_schedule_cache.slot_prime_at(slot, None).unwrap();
            let vote_key = validator_node_to_vote_keys.get(&prime).unwrap();
            assert!(progress
    ...
    fn test_replay_commitment_cache() {
        fn prime_vote(vote_slot: Slot, bank: &Arc<Bank>, pubkey: &Pubkey) {
            let mut prime_vote_account = bank.get_account(pubkey).unwrap();
            let mut vote_state = VoteState::from(&prime_vote_account).unwrap();
            vote_state.process_slot_vote_unchecked(vote_slot);
            let versioned = VoteStateVersions::new_current(vote_state);
            VoteState::to(&versioned, &mut prime_vote_account).unwrap();
            bank.store_account(pubkey, &prime_vote_account);
        }

        let prime_pubkey = initium_sdk::pubkey::new_rand();
        let prime_lamports = 3;
        let genesis_config_info =
            create_genesis_config_with_prime(50, &prime_pubkey, prime_lamports);
        let mut genesis_config = genesis_config_info.genesis_config;
        let prime_voting_pubkey = genesis_config_info.voting_keypair.pubkey();
        ...
        for i in 1..=3 {
            ...
            prime_vote(i - 1, &arc_bank, &prime_voting_pubkey);
        ...
        expected0.increase_confirmation_stake(3, prime_lamports);
        assert_eq!(
            block_commitment_cache
                .read()
                .unwrap()
        ...
        expected1.increase_confirmation_stake(2, prime_lamports);
        assert_eq!(
            block_commitment_cache
                .read()
                .unwrap()
        ...
        expected2.increase_confirmation_stake(1, prime_lamports);
        assert_eq!(
            block_commitment_cache
                .read()
                .unwrap()
    ...
    fn test_write_persist_transaction_status() {
        let GenesisConfigInfo {
            ...
            mint_keypair,
            ..
        } = create_genesis_config(initium_sdk::native_token::sol_to_lamports(1000.0));
    ...
    fn test_should_retransmit() {
        ...
        // >= 4 + NUM_CONSECUTIVE_PRIME_SLOTS, or if we reset to < 4
        ...
        for posync_slot in 4..4 + NUM_CONSECUTIVE_PRIME_SLOTS {
            assert!(!ReplayStage::should_retransmit(
                posync_slot,
                &mut last_retransmit_slot
            ));
        ...
        let posync_slot = 4 + NUM_CONSECUTIVE_PRIME_SLOTS;
        last_retransmit_slot = 4;
        assert!(ReplayStage::should_retransmit(
            posync_slot,
            &mut last_retransmit_slot
        ));
        assert_eq!(last_retransmit_slot, posync_slot);
    ...
    fn test_update_propagation_status() {
        ...
        assert!(!progress_map.get_prime_propagation_slot_must_exist(10).0);
    ...
    fn test_chain_update_propagation_status() {
        ...
        // the way back through earlier prime banks
        ReplayStage::update_propagation_status(
            &mut progress_map,
            10,
            &RwLock::new(bank_forks),
        ...
        for i in 1..=10 {
            ...
            // Only the even numbered ones were prime banks, so only
    ...
    #[test]
    fn test_check_propagation_for_start_prime() {
        let mut progress_map = ProgressMap::default();
        let posync_slot = 5;
        let parent_slot = posync_slot - NUM_CONSECUTIVE_PRIME_SLOTS;

        // If there is no previous prime slot (previous prime slot is None),
        // should succeed
        progress_map.insert(
            parent_slot,
            ForkProgress::new(Hash::default(), None, None, 0, 0),
        );
        assert!(ReplayStage::check_propagation_for_start_prime(
            posync_slot,
            parent_slot,
            &progress_map,
        ));

        // Now if we make the parent was itself the prime, then requires propagation
        // confirmation check because the parent is at least NUM_CONSECUTIVE_PRIME_SLOTS
        // slots from the `posync_slot`
        progress_map.insert(
            parent_slot,
            ForkProgress::new(
                Hash::default(),
        ...
        // Now, set up the progress map to show that the `previous_prime_slot` of 5 is
        // `parent_slot - 1` (not equal to the actual parent!), so `parent_slot - 1` needs
        // to see propagation confirmation before we can start a prime for block 5
        let previous_prime_slot = parent_slot - 1;
        progress_map.insert(
            parent_slot,
            ForkProgress::new(Hash::default(), Some(previous_prime_slot), None, 0, 0),
        );
        progress_map.insert(
            previous_prime_slot,
            ForkProgress::new(
        ...
        // `previous_prime_slot` has not seen propagation threshold, so should fail
        assert!(!ReplayStage::check_propagation_for_start_prime(
            posync_slot,
            parent_slot,
            &progress_map,
        ...
        // If we set the is_propagated = true for the `previous_prime_slot`, should
        // allow the block to be generated
        progress_map
            .get_mut(&previous_prime_slot)
            .unwrap()
            .propagated_stats
            .is_propagated = true;
        assert!(ReplayStage::check_propagation_for_start_prime(
        ...
        // If the root is now set to `parent_slot`, this filters out `previous_prime_slot` from the progress map,
        ...
        // Should purge only `previous_prime_slot` from the progress map
    ...
    fn test_check_propagation_skip_propagation_check() {
        let mut progress_map = ProgressMap::default();
        let posync_slot = 4;
        let mut parent_slot = posync_slot - 1;

        // Set up the progress map to show that the last prime slot of 4 is 3,
        // which means 3 and 4 are consecutive prime slots
        progress_map.insert(
            3,
            ForkProgress::new(
                Hash::default(),
        ...
        // If the previous prime slot has not seen propagation threshold, but
        // was the direct parent (implying consecutive prime slots), create
        ...
        // Now insert another parent slot 2 for which this validator is also the Prime Node
        parent_slot = posync_slot - NUM_CONSECUTIVE_PRIME_SLOTS + 1;
        progress_map.insert(
            parent_slot,
            ForkProgress::new(
                Hash::default(),
        ...
        // Even though `parent_slot` and `posync_slot` are separated by another block,
        ...
        // Once the distance becomes >= NUM_CONSECUTIVE_PRIME_SLOTS, then we need to
        // enforce the propagation check
        parent_slot = posync_slot - NUM_CONSECUTIVE_PRIME_SLOTS;
        progress_map.insert(
            parent_slot,
            ForkProgress::new(
                Hash::default(),
    ...
    #[test]
    fn test_prime_snapshot_restart_propagation() {
        let ReplayBlockstoreComponents {
            validator_node_to_vote_keys,
            prime_schedule_cache,
            vote_simulator,
        ...
        let root_bank = bank_forks.read().unwrap().root_bank();
        let my_pubkey = prime_schedule_cache
            .slot_prime_at(root_bank.slot(), Some(&root_bank))
            .unwrap();

        // Check that we are the prime of the root bank
        assert!(
            progress
        ...
        assert!(
            !progress
                .get_prime_propagation_slot_must_exist(root_bank.slot())
                .0
        );
    ...
    #[test]
    fn test_retransmit_latest_unpropagated_prime_slot() {
        let ReplayBlockstoreComponents {
            validator_node_to_vote_keys,
            prime_schedule_cache,
            posync_recorder,
        ...
        let posync_recorder = Arc::new(posync_recorder);
        ...
        ReplayStage::retransmit_latest_unpropagated_prime_slot(
            &posync_recorder,
            &retransmit_slots_sender,
            &mut progress,
        );
    ...
    fn test_maybe_retransmit_unpropagated_slots() {
        ...
        for i in (1..10).chain(11..15) {
            let bank = Bank::new_from_parent(
                ...
                &prime_schedule_cache.slot_prime_at(i, None).unwrap(),
                i,
            );
            progress.insert(
                i,
            ...
            assert!(progress.get_propagated_stats(i).unwrap().is_prime_slot);
            bank.freeze();
            bank_forks.write().unwrap().insert(bank);
            prev_index = i;
        }

        // expect single slot when latest_prime_slot is the start of a consecutive range
        let latest_prime_slot = 0;
