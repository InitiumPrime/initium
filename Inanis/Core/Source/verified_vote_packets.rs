use {
    ...
    initium_runtime::{bank::Bank, vote_transaction::VoteTransaction},
    initium_sdk::{
...
impl<'a> ValidatorGossipVotesIterator<'a> {
    pub fn new(
        ...
        let slot_hashes_account = my_prime_bank.get_account(&sysvar::slot_hashes::id());
        ...
        // TODO: my_prime_bank.vote_accounts() may not contain zero-staked validators
        // in this epoch, but those validators may have stake warming up in the next epoch
        let vote_account_keys: Vec<Pubkey> =
            my_prime_bank.vote_accounts().keys().copied().collect();
...
impl<'a> Iterator for ValidatorGossipVotesIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while !self.vote_account_keys.is_empty() {
            ...
            // that are:
            // 1) missing from the current prime bank
            // 2) on the same fork
...
mod tests {
    use {
        ...
        initium_perf::packet::Packet,
        initium_sdk::slot_hashes::MAX_ENTRIES,
        initium_vote_program::vote_state::Vote,
    };

    #[test]
    fn test_verified_vote_packets_receive_and_process_vote_packets() {
        let (s, r) = unbounded();
        let vote_account_key = initium_sdk::pubkey::new_rand();
    ...
    fn test_verified_vote_packets_validator_gossip_votes_iterator_wrong_fork() {
        ...
        let my_prime_bank = vote_simulator.bank_forks.read().unwrap().root_bank();
        ...
        // since they are not on the same fork as `my_prime_bank`, i.e. their hashes do
        // not exist in the SlotHashes sysvar for `my_prime_bank`
    ...
    fn test_verified_vote_packets_validator_gossip_votes_iterator_correct_fork() {
        ...
        let mut my_prime_bank = vote_simulator.bank_forks.read().unwrap().root_bank();

        // Create a set of valid ancestor hashes for this fork
        for _ in 0..MAX_ENTRIES {
            my_prime_bank = Arc::new(Bank::new_from_parent(
                &my_prime_bank,
                &Pubkey::default(),
                my_prime_bank.slot() + 1,
            ));
        }
        let slot_hashes_account = my_prime_bank
        ...
        my_prime_bank.freeze();
        let vote_slot = my_prime_bank.slot();
        let vote_hash = my_prime_bank.hash();
        let my_prime_bank = Arc::new(Bank::new_from_parent(
            &my_prime_bank,
            &Pubkey::default(),
            my_prime_bank.slot() + 1,
        ));
