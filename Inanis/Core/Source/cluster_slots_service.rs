...
impl ClusterSlotsService {
    pub fn new(
        ...
        let t_cluster_slots_service = Builder::new()
            .name("initium-cluster-slots-service".to_string())
            .spawn(move || {
                Self::run(
                    blockstore,
                    cluster_slots,
    ...
    fn initialize_lowest_slot(blockstore: &Blockstore, cluster_info: &ClusterInfo) {
        // Safe to set into gossip because by this time, the Initium Prime schedule cache should
...
mod test {
    use {
        super::*,
        initium_gossip::{cluster_info::Node, crds_value::LowestSlot},
        initium_sdk::{pubkey::Pubkey, signature::Keypair},
