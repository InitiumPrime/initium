use {
    ...
    initium_ledger::{blockstore::Blockstore, blockstore_meta::SlotMeta},
    initium_sdk::{clock::Slot, hash::Hash},
    ...
    fn add_tree_with_missing_shreds(
        ...
        while let Some(visit) = walk.get() {
            ...
            if parent.is_some() || !is_orphan {
                ...
                let entries = initium_entry::entry::create_ticks(
                ...
                let mut shreds = initium_ledger::blockstore::entries_to_test_shreds(
                    &entries,
                    slot,
