...
pub use initium_perf::report_target_features;
use {
    crate::{
        ...
        posync_timing_report_service::PosyncTimingReportService,
    ...
    rand::{thread_rng, Rng},
    initium_entry::posync::compute_hash_time_ns,
    initium_geyser_plugin_manager::geyser_plugin_service::GeyserPluginService,
    initium_gossip::{
        cluster_info::{
    ...
    initium_ledger::{
        ...
        prime_schedule::FixedSchedule,
        prime_schedule_cache::PrimeScheduleCache,
    },
    initium_measure::measure::Measure,
    initium_metrics::{datapoint_info, posync_timing_point::PosyncTimingSender},
    initium_posync::{
        posync_recorder::PosyncRecorder,
        posync_service::{self, PosyncService},
    },
    initium_rpc::{
        max_slots::MaxSlots,
    ...
    initium_sdk::{
        clock::Slot,
        epoch_schedule::MAX_PRIME_SCHEDULE_EPOCH_OFFSET,
        exit::Exit,
    ...
    initium_send_transaction_service::send_transaction_service,
    initium_streamer::socket::SocketAddrSpace,
    initium_vote_program::vote_state::VoteState,
    std::{
...
pub struct ValidatorConfig {
    ...
    pub fixed_prime_schedule: Option<FixedSchedule>,
    ...
    pub posync_verify: bool, // Perform PoSync verification during blockstore processing at boo
    ...
    pub no_posync_speed_test: bool,
    pub no_os_memory_stats_reporting: bool,
    pub no_os_network_stats_reporting: bool,
    pub posync_pinned_cpu_core: usize,
    pub posync_hashes_per_batch: u64,
    ...
    pub no_wait_for_vote_to_start_prime: bool,
...
impl Default for ValidatorConfig {
    fn default() -> Self {
        Self {
            ...
            no_posync_speed_test: true,
            no_os_memory_stats_reporting: true,
            no_os_network_stats_reporting: true,
            posync_pinned_cpu_core: posync_service::DEFAULT_PINNED_CPU_CORE,
            posync_hashes_per_batch: posync_service::DEFAULT_HASHES_PER_BATCH,
            ...
            no_wait_for_vote_to_start_prime: true,
...
pub struct Validator {
    ...
    posync_service: PosyncService,
    tpu: Tpu,
    tvu: Tvu,
    ip_echo_server: Option<initium_net_utils::IpEchoServer>,
...
impl Validator {
    ...
    pub fn new(
        ...
        if initium_perf::perf_libs::api().is_some() {
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::CleaningAccounts;
        ...
        let (
            ...
            Some(posync_timing_point_sender.clone()),
        );
        ...
        let prime_schedule_cache = Arc::new(prime_schedule_cache);
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let posync_config = Arc::new(genesis_config.posync_config.clone());
        let (posync_recorder, entry_receiver, record_receiver) = {
            let bank = &bank_forks.read().unwrap().working_bank();
            PosyncRecorder::new_with_clear_signal(
                bank.tick_height(),
                bank.last_blockhash(),
                bank.clone(),
                None,
                ...
                &prime_schedule_cache,
                &posync_config,
                Some(posync_timing_point_sender),
                exit.clone(),
            )
        };
        let posync_recorder = Arc::new(Mutex::new(posync_recorder));
        ...
        let ip_echo_server = match node.sockets.ip_echo {
            None => None,
            Some(tcp_listener) => Some(initium_net_utils::ip_echo_server(
                tcp_listener,
        ...
        let wait_for_vote_to_start_prime =
            !waited_for_supermajority && !config.no_wait_for_vote_to_start_prime;

        let posync_service = PosyncService::new(
            posync_recorder.clone(),
            &posync_config,
            &exit,
            bank_forks.read().unwrap().root_bank().ticks_per_slot(),
            config.posync_pinned_cpu_core,
            config.posync_hashes_per_batch,
            record_receiver,
        );
        assert_eq!(
    ...
    pub fn join(self) {
        ...
        self.posync_service.join().expect("posync_service");
        drop(self.posync_recorder);
        ...
        self.posync_timing_report_service
            .join()
            .expect("posync_timing_report_service");
    }
}
...
fn check_posync_speed(genesis_config: &GenesisConfig, maybe_hash_samples: Option<u64>) {
    if let Some(hashes_per_tick) = genesis_config.hashes_per_tick() {
        ...
        if my_ns_per_slot < target_ns_per_slot {
            ...
            info!("PoSync speed check: Will sleep {}ns per slot.", extra_ns);
        } else {
            error!(
                "PoSynch is slower than cluster target tick rate! mine: {} cluster: {}. If you wish to continue, try --no-posync-speed-test",
...
fn load_blockstore(
    ...
    posync_timing_point_sender: Option<PosyncTimingSender>,
) -> (
    GenesisConfig,
    ...
    *start_progress.write().unwrap() = ValidatorStartProgress::LoadingLedger;
    ...
    // grows too large
    let prime_schedule_slot_offset = genesis_config.epoch_schedule.prime_schedule_slot_offset;
    let slots_per_epoch = genesis_config.epoch_schedule.slots_per_epoch;
    let prime_epoch_offset = (prime_schedule_slot_offset + slots_per_epoch - 1) / slots_per_epoch;
    assert!(prime_epoch_offset <= MAX_PRIME_SCHEDULE_EPOCH_OFFSET);
    ...
    if !config.no_posync_speed_test {
        check_posync_speed(&genesis_config, None);
    }
    ...
    blockstore.shred_timing_point_sender = posync_timing_point_sender;
    ...
    let process_options = blockstore_processor::ProcessOptions {
        posync_verify: config.posync_verify,
        halt_at_slot,
    ...
    let (bank_forks, mut prime_schedule_cache, starting_snapshot_hashes) =
    ...
    prime_schedule_cache.set_fixed_prime_schedule(config.fixed_prime_schedule.clone());
    {
...
struct ProcessBlockStore<'a> {
    ...
    prime_schedule_cache: &'a PrimeScheduleCache,
...
fn maybe_warp_slot(
    ...
    if let Some(warp_slot) = config.warp_slot {
        ...
        bank_forks.set_root(
            warp_slot,
            &initium_runtime::accounts_background_service::AbsRequestSender::default(),
            Some(warp_slot),
        );
        prime_schedule_cache.set_root(&bank_forks.root_bank());
...
mod tests {
    use {
        ...
        initium_ledger::{create_new_tmp_ledger, genesis_utils::create_genesis_config_with_prime},
        initium_sdk::{genesis_config::create_genesis_config, posync_config::PoSyncConfig},
    ...
    fn validator_exit() {
        ...
        let prime_keypair = Keypair::new();
        let prime_node = Node::new_localhost_with_pubkey(&prime_keypair.pubkey());

        let validator_keypair = Keypair::new();
        let validator_node = Node::new_localhost_with_pubkey(&validator_keypair.pubkey());
        let genesis_config =
            create_genesis_config_with_prime(10_000, &prime_keypair.pubkey(), 1000)
                .genesis_config;
    ...
    fn test_backup_and_clear_blockstore() {
        ...
        use {
            initium_entry::entry,
            initium_ledger::{blockstore, get_tmp_ledger_path},
        };
    ...
    #[test]
    #[should_panic]
    fn test_posync_speed() {
        initium_logger::setup();
        let posync_config = PosyncConfig {
            target_tick_duration: Duration::from_millis(initium_sdk::clock::MS_PER_TICK),
            // make PoSync rate really fast to cause the panic condition
            hashes_per_tick: Some(100 * initium_sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PosyncConfig::default()
        };
        let genesis_config = GenesisConfig {
            posync_config,
            ..GenesisConfig::default()
        };
        check_posync_speed(&genesis_config, Some(10_000));
    }

    #[test]
    fn test_posync_speed_no_hashes_per_tick() {
        let posync_config = PosyncConfig {
            target_tick_duration: Duration::from_millis(initium_sdk::clock::MS_PER_TICK),
