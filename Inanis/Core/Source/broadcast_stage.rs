//! A stage to broadcast data from a Initium Prime node to validators
...
use {
    ...
    itertools::Itertools,
    initium_gossip::cluster_info::{ClusterInfo, ClusterInfoError, DATA_PLANE_FANOUT},
    initium_ledger::{blockstore::Blockstore, shred::Shred},
    initium_measure::measure::Measure,
    initium_metrics::{inc_new_counter_error, inc_new_counter_info},
    initium_poh::poh_recorder::WorkingBankEntry,
    initium_runtime::bank_forks::BankForks,
    initium_sdk::{
        clock::Slot,
        pubkey::Pubkey,
        signature::Keypair,
...
impl BroadcastStage {
    ...
    /// Service to broadcast messages from the prime to layer 1 nodes.
    ...
    fn new(
        ...
        let thread_hdl = {
            ...
            Builder::new()
                .name("initium-broadcaster".to_string())
                .spawn(move || {
        ...
        for sock in socks.into_iter() {
            ...
            let t = Builder::new()
                .name("initium-broadcaster-transmit".to_string())
                .spawn(move || loop {
                    let res =
                        bs_transmit.transmit(&socket_receiver, &cluster_info, &sock, &bank_forks);
                    let res = Self::handle_error(res, "initium-broadcaster-transmit");
                    if let Some(res) = res {
                        return res;
                    }
                })
        ...
        for _ in 0..NUM_INSERT_THREADS {
            ...
            let btree = blockstore.clone();
            let t = Builder::new()
                .name("initium-broadcaster-record".to_string())
                .spawn(move || loop {
                    let res = bs_record.record(&blockstore_receiver, &btree);
                    let res = Self::handle_error(res, "initium-broadcaster-record");
                    if let Some(res) = res {
                        return res;
                    }
                })
        ...
        let retransmit_thread = Builder::new()
            .name("initium-broadcaster-retransmit".to_string())
            .spawn(move || loop {
                if let Some(res) = Self::handle_error(
                    ...
                    "initium-broadcaster-retransmit-check_retransmit_signals",
                ) {
                    return res;
                }
            })
...
/// broadcast messages from the prime to layer 1 nodes
/// # Remarks
...
pub mod test {
    use {
        ...
        initium_entry::entry::create_ticks,
    ...
    fn setup_dummy_broadcast_service(
        ...
        // Make the prome node and scheduler
        let prime_info = Node::new_localhost_with_pubkey(prime_pubkey);
