...
impl ClusterNodes<RetransmitStage> {
    pub(crate) fn get_retransmit_addrs(
        ...
        let (neighbors, children) =
            self.get_retransmit_peers(slot_prime, shred, root_bank, fanout);
    ...
    pub fn get_retransmit_peers(
        ...
        let shred_seed = shred.seed(slot_prime);
        let mut weighted_shuffle = self.weighted_shuffle.clone();
        // Exclude slot prime from list of nodes.
        if slot_prime == self.pubkey {
            error!("retransmit from slot prime: {}", slot_prime);
        } else if let Some(index) = self.index.get(&slot_prime) {
...
// 3rd layer: ...
// ...
// The Initium Prime node broadcasts shreds to the root node.
...
impl<T: 'static> ClusterNodesCache<T> {
    ...
    pub(crate) fn get(
        ...
        let epoch = root_bank.get_prime_schedule_epoch(shred_slot);
        ...
        if epoch_staked_nodes.is_none() {
            ...
            if epoch != root_bank.get_prime_schedule_epoch(root_bank.slot()) {
