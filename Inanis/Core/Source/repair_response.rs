use {
    ...
    initium_perf::packet::limited_deserialize,
    initium_sdk::{clock::Slot, packet::Packet},
...
mod test {
    ...
    fn run_test_sigverify_shred_cpu_repair(slot: Slot) {
        ...
        let prime_slots = [(slot, keypair.pubkey().to_bytes())]
            .iter()
            .cloned()
            .collect();
        let rv = verify_shred_cpu(&packet, &prime_slots);
        assert_eq!(rv, Some(1));

        let wrong_keypair = Keypair::new();
        let prime_slots = [(slot, wrong_keypair.pubkey().to_bytes())]
            .iter()
            .cloned()
            .collect();
        let rv = verify_shred_cpu(&packet, &prime_slots);
