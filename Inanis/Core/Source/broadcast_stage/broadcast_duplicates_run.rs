use {
    ...
    initium_gossip::cluster_info::DATA_PLANE_FANOUT,
    initium_ledger::shred::Shredder,
    initium_sdk::{
        hash::Hash,
...
pub struct BroadcastDuplicatesConfig {
    /// Amount of stake (excluding the prime) to send different version of slots to.
...
impl BroadcastRun for BroadcastDuplicatesRun {
    fn run(
        ...
        // 2) Convert entries to shreds + generate coding shreds. Set a garbage PoSync on the last entry
    ...
    fn transmit(
        ...
        let cluster_partition: HashSet<Pubkey> = {
            ...
            let epoch = root_bank.get_prime_schedule_epoch(slot);
            root_bank
        ...
        let packets: Vec<_> = shreds
            ...
            .filter_map(|shred| {
                    ...
                    .remove(&shred.signature())
                {
                    ...
                    // is small such as in `test_duplicate_shreds_broadcast_prime()`, then
