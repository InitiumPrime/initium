...
use {
    ...
    initium_metrics::{inc_new_counter_debug, inc_new_counter_error},
    initium_perf::packet::{Packet, PacketBatch},
    initium_rayon_threadlimit::get_thread_count,
    initium_runtime::{bank::Bank, bank_forks::BankForks},
    initium_sdk::{clock::Slot, packet::PACKET_DATA_SIZE, pubkey::Pubkey},
    std::{
        cmp::Reverse,
...
/// drop shreds that are from myself or not from the correct prime for the
/// shred's slot
pub(crate) fn should_retransmit_and_persist(
    ...
    let slot_prime_pubkey = prime_schedule_cache.slot_prime_at(shred.slot(), bank.as_deref());
    if let Some(prime_id) = slot_prime_pubkey {
        if prime_id == *my_pubkey {
        ...
        inc_new_counter_debug!("streamer-recv_window-unknown_prime", 1);
        false
    }
}
...
impl WindowService {
    ...
    fn start_check_duplicate_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("initium-check-duplicate-error", 1, 1);
        };
        Builder::new()
            .name("initium-check-duplicate".to_string())
            .spawn(move || loop {
    ...
    fn start_window_insert_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("initium-window-insert-error", 1, 1);
        };

        Builder::new()
            .name("initium-window-insert".to_string())
            .spawn(move || {
            ...
            + std::marker::Sync,
    {
        ...
        Builder::new()
            ...
            .spawn(move || {
                ...
                let handle_error = || {
                    inc_new_counter_error!("initium-window-error", 1, 1);
                };
...
mod test {
    use {
        super::*,
        initium_entry::entry::{create_ticks, Entry},
        initium_gossip::contact_info::ContactInfo,
        initium_ledger::{
            blockstore::{make_many_slot_entries, Blockstore},
            genesis_utils::create_genesis_config_with_prime,
            get_tmp_ledger_path,
            shred::Shredder,
        },
        initium_sdk::{
    ...
    fn test_should_retransmit_and_persist() {
        let me_id = initium_sdk::pubkey::new_rand();
        let prime_keypair = Arc::new(Keypair::new());
        let prime_pubkey = prime_keypair.pubkey();
        let bank = Arc::new(Bank::new_for_tests(
            &create_genesis_config_with_prime(100, &prime_pubkey, 10).genesis_config,
        ));
        let cache = Arc::new(PrimeScheduleCache::new_from_bank(&bank));

        let shreds = local_entries_to_shred(&[Entry::default()], 0, 0, &prime_keypair);
        ...
        // substitute prime_pubkey for me_id so it looks I was the prime
        ...
        // change the shred's slot so prime lookup fails
        // with a Bank and no idea who prime is, shred gets thrown out
        ...
        let shreds = local_entries_to_shred(&[Entry::default()], root, root - 1, &prime_keypair);
        assert!(!should_retransmit_and_persist(
            &shreds[0],
            Some(bank.clone()),
            &cache,
        ...
        let root = MINIMUM_SLOTS_PER_EPOCH as u64 * 3;
        let shreds =
            local_entries_to_shred(&[Entry::default()], root + 1, root - 1, &prime_keypair);
        assert!(!should_retransmit_and_persist(
            &shreds[0],
            Some(bank.clone()),
            &cache,
        ...
        coding_shred.sign(&prime_keypair);
