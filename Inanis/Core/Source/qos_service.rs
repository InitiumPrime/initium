...
impl QosService {
    pub fn new(cost_model: Arc<RwLock<CostModel>>, id: u32) -> Self {
        ...
        let reporting_thread = Some(
            Builder::new()
                .name("initium-qos-service-metrics-repoting".to_string())
                .spawn(move || {
...
struct QosServiceMetricsStats {
    ...
    /// usually the number of sanitized transactions prime node receives.
...
mod tests {
    use {
        super::*,
        itertools::Itertools,
        initium_runtime::genesis_utils::{create_genesis_config, GenesisConfigInfo},
        initium_sdk::{
            hash::Hash,
