use {
    ...
    initium_ledger::blockstore_processor::{ConfirmationProgress, ConfirmationTiming},
    initium_program_runtime::timings::ExecuteTimingType,
    initium_runtime::{bank::Bank, bank_forks::BankForks, vote_account::VoteAccountsHashMap},
    initium_sdk::{clock::Slot, hash::Hash, pubkey::Pubkey},
    std::{
...
impl ReplaySlotStats {
    pub fn report_stats(&self, slot: Slot, num_entries: usize, num_shreds: u64) {
        datapoint_info!(
            ...
            (
                "entry_posync_verification_time",
                self.posync_verify_elapsed as i64,
                i64
            ),
            (
...
pub struct PropagatedStats {
    ...
    pub is_prime_slot: bool,
    pub prev_prime_slot: Option<Slot>,
...
impl ProgressMap {
    ...
    pub fn get_latest_prime_slot_must_exist(&self, slot: Slot) -> Option<Slot> {
        let propagated_stats = self.get_propagated_stats_must_exist(slot);
        if propagated_stats.is_prime_slot {
            Some(slot)
        } else {
            propagated_stats.prev_prime_slot
        }
    }

    pub fn get_prime_propagation_slot_must_exist(&self, slot: Slot) -> (bool, Option<Slot>) {
        if let Some(prime_slot) = self.get_latest_prime_slot_must_exist(slot) {
            // If the prime's stats are None (isn't in the
            // progress map), this means that prev_prime slot is
            // rooted, so return true
            (
                self.is_propagated(prime_slot).unwrap_or(true),
                Some(prime_slot),
            )
        } else {
            // prev_prime_slot doesn't exist because already rooted
            // or this validator hasn't been scheduled as a prime
            // yet. In both cases the latest prime is vacuously
            // confirmed
            (true, None)
        }
    }
    ...
    pub fn get_bank_prev_prime_slot(&self, bank: &Bank) -> Option<Slot> {
        let parent_slot = bank.parent_slot();
        self.get_propagated_stats(parent_slot)
            .map(|stats| {
                if stats.is_prime_slot {
                    Some(parent_slot)
                } else {
                    stats.prev_prime_slot
                }
            })
            .unwrap_or(None)
    }
...
#[cfg(test)]
mod test {
    use {super::*, initium_runtime::vote_account::VoteAccount, initium_sdk::account::Account};

    fn new_test_vote_account() -> VoteAccount {
        let account = Account {
            owner: initium_vote_program::id(),
            ..Account::default()
        };
    ...
    fn test_add_vote_pubkey() {
        ...
        let mut vote_pubkey = initium_sdk::pubkey::new_rand();
        ...
        vote_pubkey = initium_sdk::pubkey::new_rand();
    ...
    fn test_add_node_pubkey_internal() {
        ...
        let vote_account_pubkeys: Vec<_> = std::iter::repeat_with(initium_sdk::pubkey::new_rand)
        ...
        let mut stats = PropagatedStats::default();
        let mut node_pubkey = initium_sdk::pubkey::new_rand();
        ...
        // shouldn't increase
        node_pubkey = initium_sdk::pubkey::new_rand();
        stats.add_node_pubkey_internal(&node_pubkey, &vote_account_pubkeys, &epoch_vote_accounts);
        assert!(stats.propagated_node_ids.contains(&node_pubkey));
        assert_eq!(
            stats.propagated_validators_stake,
    ...
    fn test_is_propagated_status_on_construction() {
        ...
        // a prime slot and is_propagated == false
    ...
    fn test_is_propagated() {
        ...
        // Insert new ForkProgress for slot 10 (not a prime slot) and its
        // previous prime slot 9 (prime slot)
        ...
        assert!(!progress_map.get_prime_propagation_slot_must_exist(9).0);
        assert!(!progress_map.get_prime_propagation_slot_must_exist(10).0);

        // Insert new ForkProgress for slot 8 with no previous prime.
        // The previous prime before 8, slot 7, does not exist in
        ...
        assert!(progress_map.get_prime_propagation_slot_must_exist(8).0);
        ...
        // is the last prime slot before 10
        assert!(progress_map.get_prime_propagation_slot_must_exist(10).0);

        // If we make slot 10 a prime slot though, even though its previous
        // prime slot 9 has been confirmed, slot 10 itself is not confirmed
        progress_map
