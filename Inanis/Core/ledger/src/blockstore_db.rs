...
use {
    ...
    initium_runtime::hardened_unpack::UnpackError,
    initium_sdk::{
    ...
    fn open(path: &Path, options: BlockstoreOptions) -> Result<Rocks> {
        ...
        let db = match access_type {
            ...
            AccessType::Secondary => {
                let secondary_path = path.join("initium-secondary");

                info!(
                    "Opening Rocks with secondary (read only) access at: {:?}",
                    secondary_path
                );
                info!("This secondary access could temporarily degrade other accesses, such as by initium-validator");
        ...
        // This is only needed by initium-validator for LedgerCleanupService so guard with AccessType::Primary
